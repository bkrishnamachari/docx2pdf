...
try {
    ...
    $doc.ExportAsFixedFormat("<absolute-pdf-path>", 17, $false, 0, 0, 0, 0, 0, $false, $false, 0, $false, $true, $false)
    $doc.Close($false)
} finally {
    $word.Quit()
...
IMPORTANT: Set a 60-second timeout on the conversion command. The `start` command MUST use the `start "" "path"` format (empty string for window title).
